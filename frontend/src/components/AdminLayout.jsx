...
const NAV_ITEMS = [
  ...
  {
    to: '/admin/ki-angebote',
    label: 'KI-Angebots-Assistent',
  ...
  { separator: true, label: 'Hilfe' },
  {
    to: '/admin/hilfe',
    label: 'Hilfe & Handbuch',
    icon: (
  ...
  {
    to: '/admin/abo',
    label: 'Meine Mitgliedschaft',
    icon: (
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h18M7 15h1m4 0h1m-8 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" />
    ),
  },
  {
    to: '/admin/firma',
  ...
  {
    to: '/admin/dokumentlayout',
    label: 'Dokumentlayout',
    permission: 'company_manage',
    feature: 'quotes',
    icon: (
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
...
export default function AdminLayout() {
  ...
  return (
    <div className="admin-shell flex min-h-[calc(100vh-72px)]">
      ...
      `}>
        <nav className="flex-1 px-3 py-4 space-y-1">
          {NAV_ITEMS.map((item, idx) => {
            if (item.separator) {
              const hasVisibleItemsAhead = NAV_ITEMS.slice(idx + 1).some((entry) => !entry.separator && (!entry.permission || hasPermission(entry.permission)) && (!entry.feature || hasFeature(entry.feature)));
      ...
      {/* Schwebender KI-Assistent (nur mit Angebots-/Quotes-Feature) */}
      {hasFeature('quotes') && <AssistantChat />}
