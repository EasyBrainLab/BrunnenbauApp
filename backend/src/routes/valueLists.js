...
router.get('/:key/items', (req, res) => {
  const db = getDb();
  const list = db.prepare('SELECT id FROM value_lists WHERE list_key = ?').get(req.params.key);
...
router.post('/', requireAuth, (req, res) => {
  ...
  const db = getDb();
  try {
    db.prepare('INSERT INTO value_lists (list_key, display_name, description, is_system) VALUES (?, ?, ?, 0)').run(list_key, display_name, description || null);
    const created = db.prepare('SELECT * FROM value_lists WHERE list_key = ?').get(list_key);
...
router.put('/:key', requireAuth, (req, res) => {
  ...
  const db = getDb();
  const result = db.prepare('UPDATE value_lists SET display_name = COALESCE(?, display_name), description = COALESCE(?, description) WHERE list_key = ?')
    .run(display_name || null, description !== undefined ? description : null, req.params.key);
...
router.delete('/:key', requireAuth, (req, res) => {
  const db = getDb();
  const list = db.prepare('SELECT * FROM value_lists WHERE list_key = ?').get(req.params.key);
  if (!list) return res.status(404).json({ error: 'Liste nicht gefunden' });
...
router.post('/:key/items', requireAuth, (req, res) => {
  ...
  try {
    db.prepare(
      'INSERT INTO value_list_items (list_id, value, label, sort_order, is_active, color, icon, metadata_json) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
    ).run(list.id, value, label, sort_order || 0, is_active !== undefined ? (is_active ? 1 : 0) : 1, color || null, icon || null, metadata_json || null);
...
router.put('/:key/items/:id', requireAuth, (req, res) => {
  ...
  const item = db.prepare('SELECT vli.* FROM value_list_items vli JOIN value_lists vl ON vl.id = vli.list_id WHERE vl.list_key = ? AND vli.id = ?')
    .get(req.params.key, req.params.id);
